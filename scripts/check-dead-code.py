...
Identifies unused exports, imports, functions, and variables in TypeScript/JavaScript codebases.
Uses AST parsing for accuracy and provides actionable reports with file:line references.
...
class DeadCodeError:
    """Represents a dead code violation."""
...
class DeadCodeChecker:
    ...
    def __init__(self, target_path: str = "src"):
        ...
        self.errors: List[DeadCodeError] = []
        self.warnings: List[DeadCodeError] = []
    ...
    def _check_unused_exports(self) -> None:
        """Check for unused exports across the codebase."""
        print("Checking for unused exports...")
        ...
        for key, export in all_exports.items():
            if key not in imported_exports and not export.is_reexport:
                ...
                unused_exports.append(export)
        
        # Report unused exports
        if unused_exports:
            for export in unused_exports:
                self.warnings.append(DeadCodeError(
                    f"⚠️  Unused export '{export.name}' in {export.file_path.relative_to(self.target_path)}:{export.line_number}",
                    severity="warning"
                ))
        ...
        for file_analysis in self.file_cache.values():
            for imp in file_analysis.imports:
                if imp.name not in file_analysis.used_symbols:
                    ...
                    self.warnings.append(DeadCodeError(
                        f"⚠️  Unused import '{imp.name}' in {imp.file_path.relative_to(self.target_path)}:{imp.line_number}",
                        severity="warning"
                    ))
    
    def _check_unused_local_symbols(self) -> None:
        """Check for unused local symbols within files."""
        print("Checking for unused local symbols...")
        
        for file_analysis in self.file_cache.values():
            for symbol in file_analysis.symbols:
                if not symbol.is_exported and symbol.name not in file_analysis.used_symbols:
                    ...
                    self.errors.append(DeadCodeError(
                        f"❌ Unused {symbol.symbol_type} '{symbol.name}' in {symbol.file_path.relative_to(self.target_path)}:{symbol.line_number}"
                    ))
    
    def run_all_checks(self) -> bool:
        """Run all dead code checks and return True if no critical errors."""
        start_time = time.time()
        
        print(f"🕵️  Checking for dead code in {self.target_path}...")
        ...
        self._check_unused_local_symbols()
        ...
        if not self.errors:  # Only warnings
            print("✅ Dead code check passed (warnings only)!")
            return True
